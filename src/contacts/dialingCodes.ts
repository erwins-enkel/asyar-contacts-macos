...
//
// Used for one thing only: turning the Mac's own region (which the helper
// reads from `NSLocale`) into the prefix that promotes a nationally-stored
// number like "0172/1234567" to "+491721234567". Numbers that already carry
// a "+" or a "00" never touch this table.
//
// NANP members all map to "1"; the shared codes (7 for RU/KZ, 39 for IT/VA,
// 61 for AU/CC/CX, 212 for MA/EH, 262 for RE/YT, 590 for GP/BL/MF, 599 for
// CW/BQ) are correct in both directions here because we only ever look up a
// region to get a code, never the reverse.
export const DIALING_CODES: Readonly<Record<string, string>> = {
  ...
  SZ: '268', TC: '1', TD: '235', TG: '228', TH: '66', TJ: '992', TK: '690', TL: '670',
  TM: '993', TN: '216', TO: '676', TR: '90', TT: '1', TV: '688', TW: '886', TZ: '255',
  UA: '380', UG: '256', US: '1', UY: '598', UZ: '998', VA: '39', VC: '1', VE: '58',
  VG: '1', VI: '1', VN: '84', VU: '678', WF: '681', WS: '685', XK: '383', YE: '967',
  YT: '262', ZA: '27', ZM: '260', ZW: '263',
};

/** `"DE"` → `"+49"`. Returns `''` for an unknown or empty region, which the
 *  caller reads as "leave national numbers alone". */
export function dialingPrefixForRegion(region: string): string {
  const code = DIALING_CODES[region.trim().toUpperCase()];
  return code ? `+${code}` : '';
}
